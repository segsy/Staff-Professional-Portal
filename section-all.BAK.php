...
<?php
$query = query("SELECT c.title AS cs_title, ct.id AS c_id, ct.sort, ct.title AS c_title, ct.date AS c_date FROM training_section AS ct 
...
while($rows=mysqli_fetch_array($query)){
 // print_r($rows);
     ...
     ?>
                    ...
                    <tr class="cat<?=$c_id?>">
                      <td><?=$sort?>: <?=substr($c_title, 0, 30)?>.. </td>
                      <td><?=substr($course_title, 0, 35)?>...</td> 
                      <td>
