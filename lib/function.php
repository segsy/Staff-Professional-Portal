...
function getAllProfessional(){
    global $dbCon;
    global $tbl_proffessional;
    $op = "<option value=\"\"> Select A Category</option>";
    // $op .= "<option value='999999'> [ Available to all Staff ]</option>";

$query=query("SELECT * FROM $tbl_proffessional ORDER BY date ASC");
$count=mysqli_num_rows($query);
if($count){
while($row=mysqli_fetch_array($query))
   {
...
function getAllCategoryJFM(){
    global $dbCon;
    global $tbl_category_jfm;
    $op = "<option value=\"\"> Select A Category</option>";

$query=query("SELECT * FROM $tbl_category_jfm ORDER BY date ASC");
$count=mysqli_num_rows($query);
if($count){
while($row=mysqli_fetch_array($query))
   {
...
function getCourseNameById($i){
$query=query("SELECT title FROM training_course WHERE id = '$i' ");
$count=mysqli_num_rows($query);
if($count){
while($row=mysqli_fetch_array($query))
   {
...
function getLessonNameById($i){
$query=query("SELECT title FROM training_lesson WHERE id = '$i' ");
$count=mysqli_num_rows($query);
if($count){
while($row=mysqli_fetch_array($query))
   {
