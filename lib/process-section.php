...
switch ($_a) {
    ...
    $_i        = isset( $_POST['d'] ) ? $_POST['d'] : '';
             ...
             switch ($s) {
                    ...
                    if(query("UPDATE training_lesson SET status='1', schedule=NOW() WHERE id='$_i'")){
                                $json['success'] = 'Lesson Scheduled!';
                        }else{
                    ...
                    case 1:  // please remove me             
                        if(query("UPDATE training_lesson SET status='0', schedule='0000-00-00 00:00:00' WHERE id='$_i'")){
        ...
        break;
     case 'autoschedule':
    $_d        = isset( $_POST['schedule_date'] ) ? $_POST['schedule_date'] : '';
    $_c       = isset( $_POST['schedule_course'] ) ? $_POST['schedule_course'] : '';
//    $_l       = isset( $_POST['_h'] ) ? $_POST['_h'] : '';

            if( empty($_h)) {
                $json['error']['lesson'] = 'you didnt select a lesson';
            }
            if( !isset( $json['error'] ) ) {
                foreach($_h as $check) {
                    query("UPDATE training_lesson SET status='1', schedule='$_d' WHERE id='$check'");
                    }
                   $json['success'] = 'Lesson Scheduled Added!';
               }
 ...
 case 'new':
            ...
            if( !isset( $json['error'] ) ) {
                if(query("INSERT INTO training_section(title,status,date,course,sort) VALUES('$_t','1',NOW(),'$_c','$_s')")){
