...
<?php
/*
SELECT c.id AS c_id, c.duration AS c_duration, c.title AS c_title, c.duration AS c_duration,
DATE_FORMAT(c.date, '%a, %b %y') AS c_date, COUNT(e.id) AS p_enrolled
FROM training_course AS c
                LEFT JOIN training_enrollment AS e ON c.id = e.course
                GROUP BY c.id
