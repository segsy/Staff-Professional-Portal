 ...
 switch ($a_) {
   ...
   echo"# lesson started...";
        ...
        if(mysqli_num_rows(query("SELECT * FROM $tbl_record WHERE user='$_id' AND lesson='$l' AND course='$cs'"))){
         query("UPDATE $tbl_record SET progress = '$p' WHERE user='$_id' AND lesson='$l' AND course='$cs' AND progress != 'd'");

        }else{
        query("INSERT INTO $tbl_record(lesson,course,dept,user,date,progress,start,end) VALUES('$l','$cs','$_dept','$_id',NOW(),'0',NOW(),'')");
        }
