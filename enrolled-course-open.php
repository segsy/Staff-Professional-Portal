...
/*
SELECT e.course, e.id AS p_id, d.id AS d_id, d.name AS d_name, COUNT(d.id) AS d_record
FROM training_enrollment AS e
 LEFT JOIN training_dept AS d ON (e.dept = d.id)
 WHERE e.course='zIAStXorPPsODcwjKYDV' AND e.dept IS NOT NULL
GROUP BY d.id ORDER BY d_record DESC

SELECT p.course, p.id AS p_id, d.id AS d_id, d.name AS d_name, COUNT(d.id) AS d_record, SUM(p.progress  = '100' ) AS p_completed,  SUM(p.progress  != '100' ) AS p_inprogresss FROM training_program AS p
 LEFT JOIN training_dept AS d ON (p.dept = d.id)
 WHERE p.course='$_i' AND p.dept IS NOT NULL
GROUP BY d.id ORDER BY d_record DESC
*/
